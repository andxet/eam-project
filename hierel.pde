  ...
  float[] x_positioning (float leftmost, float rightmost) {
    ...
    if (daughter_list.size()>0) {
      for (int i=0; i < daughter_list.size(); i++) { // for each daughter 
        ...
        if (d.type.equals("TD")) { // if terminal ... DrawingObj_v d_t;
          ...
          if (lm_rm[0]==-1 || lm_rm[0] > t_s.d_t.x_coord - incident_side/2 - incident_hdistance/2) {lm_rm[0] = t_s.d_t.x_coord - incident_side/2 - incident_hdistance/2;}; 
          if (lm_rm[1] < t_s.d_t.x_coord + incident_side/2 + incident_hdistance/2) {lm_rm[1] = t_s.d_t.x_coord + incident_side/2 + incident_hdistance/2;};
          ...
          if (lm_rm[0]==-1 || lm_rm[0] > he.hierel_mt.d_nt.x_coord - incident_side/2 - incident_hdistance/2) {lm_rm[0] = he.hierel_mt.d_nt.x_coord - incident_side/2 - incident_hdistance/2;}; 
          if (lm_rm[1] < he.hierel_mt.d_nt.x_coord + incident_side/2 + incident_hdistance/2) {lm_rm[1] = he.hierel_mt.d_nt.x_coord + incident_side/2 + incident_hdistance/2;};
        } // END IF
      } // END FOR
    ...
    else // ALIGNMENT ON UNITS (NON TERMINALS)
    {
      if (!(mapping_init.equals("NULL"))) {
        ...
        lm_rm[0] = nt_s.d_unit.x_coord - incident_side/2 - incident_hdistance/2; 
        if (!(mapping_end.equals("NULL"))) {        
          ...
          lm_rm[1] = nt_s2.d_unit.x_coord + incident_side/2 + incident_hdistance/2;
        }
      }
    }
